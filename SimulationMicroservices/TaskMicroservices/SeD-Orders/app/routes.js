...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "SeD-Orders", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.put('/confirmsalesorder', function (req, res) {
        insertQueue("PUT-/confirmsalesorder",req,res);
    });
    app.post('/newsalesorder', function (req, res) {
        insertQueue("POST-/newsalesorder",req,res);
    });

    app.put('/addsalesorder/:availability_id', function (req, res){
        insertQueue("PUT-/addsalesorder",req,res);
    });

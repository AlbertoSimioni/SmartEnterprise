...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "SeD-Catalogs", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/currentcatalogs', function (req, res) {
        insertQueue("GET-/currentcatalogs",req,res);
    });
    app.post('/newcatalog', function (req, res) {
        insertQueue("POST-/newcatalog",req,res);
    });
    app.put('/addcatalog/:availability_id', function (req, res){
        insertQueue("PUT-/addcatalog",req,res);
    });

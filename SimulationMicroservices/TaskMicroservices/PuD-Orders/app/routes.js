...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "PuD-Orders", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.put('/confirmpurchasingorder', function (req, res) {
        insertQueue("PUT-/confirmpurchasingorder",req,res);
    });

    app.post('/newpurchasingorder', function (req, res) {
        insertQueue("POST-/newpurchasingorder",req,res);
    });

    app.put('/addpurchasingorder/:availability_id', function (req, res){
        insertQueue("PUT-/addpurchasingorder",req,res);
    });

    app.delete('/purchasingorder/:availability_id', function (req, res) {
        insertQueue("DELETE-/purchasingorder",req,res);
    });

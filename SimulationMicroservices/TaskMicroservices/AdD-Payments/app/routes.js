...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "AdD-Payments", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/currentavailabilities', function (req, res) {
        insertQueue("GET-/currentavailabilities",req,res);
    });

    app.post('/newAvailability', function (req, res) {
        insertQueue("POST-/newAvailability",req,res);

    });

    app.put('/addAvailability/:availability_id', function (req, res){
        insertQueue("PUT-/addAvailability",req,res);
    });

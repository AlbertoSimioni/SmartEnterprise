...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "LoD-Warehouses", "apiName": request.type,"date":datestring,
        ...
        .headers({'Accept': 'application/json', 'Content-Type': 'application/json'})
        .type('json')
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+5, 
            "timing" : msElapsed ,"serviceName": "LoD-Warehouses", "apiName": request.type,"date":datestring
        })
        .end();
...
module.exports = function (app) {
    ...
    app.get('/currentwarehousesstate', function (req, res) {
        insertQueue("GET-/currentwarehousesstate",req,res);
    });

...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Invoices", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/invoice/:invoice_id', function (req, res) {
        insertQueue("GET-/invoice",req,res);
    });

    app.post('/invoice', function (req, res) {
        insertQueue("POST-/invoice",req,res);
    });

    app.put('/invoice/:invoice_id', function (req, res){
        insertQueue("PUT-/invoice",req,res);
    });


    app.delete('/invoice/:invoice_id', function (req, res) {
        insertQueue("DELETE-/invoice",req,res);
    });

    app.get('/invoices', function(req,res){
        insertQueue("GET-/invoices",req,res)
    });

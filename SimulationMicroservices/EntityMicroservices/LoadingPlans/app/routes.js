...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "LoadingPlans", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/loadingplan/:loadingplan_id', function (req, res) {
        insertQueue("GET-/loadingplan",req,res);
    });

    app.post('/loadingplan', function (req, res) {
        insertQueue("POST-/loadingplan",req,res);
    });

    app.put('/loadingplan/:loadingplan_id', function (req, res){
        insertQueue("PUT-/loadingplan",req,res);
    });
    ...
    app.get('/loadingplans', function(req,res){
        insertQueue("GET-/loadingplans",req,res)
    });

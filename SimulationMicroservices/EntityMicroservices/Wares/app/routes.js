...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Wares", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/ware/:ware_id', function (req, res) {
        insertQueue("GET-/ware",req,res);
    });

    app.post('/ware', function (req, res) {
        insertQueue("POST-/ware",req,res);
    });

    app.put('/ware/:ware_id', function (req, res){
        insertQueue("PUT-/ware",req,res);
    });


    app.delete('/ware/:ware_id', function (req, res) {
        insertQueue("DELETE-/ware",req,res);
    });


    app.get('/activewares', function(req,res){
        insertQueue("GET-/activewares",req,res);
    });

    app.get('/wares', function(req,res){
        insertQueue("GET-/wares",req,res)
    });

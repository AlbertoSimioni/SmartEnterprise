...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Paymentsout", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/paymentout/:paymentout_id', function (req, res) {
        insertQueue("GET-/paymentout",req,res);
    });

    app.post('/paymentout', function (req, res) {
        insertQueue("POST-/paymentout",req,res);
    });

    app.put('/paymentout/:paymentout_id', function (req, res){
        insertQueue("PUT-/paymentout",req,res);
    });


    app.delete('/paymentout/:paymentout_id', function (req, res) {
        insertQueue("DELETE-/paymentout",req,res);
    });

    app.get('/paymentouts', function(req,res){
        insertQueue("GET-/paymentsout",req,res)
    });

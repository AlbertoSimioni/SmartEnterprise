...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Providers", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/provider/:provider_id', function (req, res) {
        insertQueue("GET-/provider",req,res);
    });

    app.post('/provider', function (req, res) {
        insertQueue("POST-/provider",req,res);
    });

    app.put('/provider/:provider_id', function (req, res){
        insertQueue("PUT-/provider",req,res);
    });


    app.delete('/provider/:provider_id', function (req, res) {
        insertQueue("DELETE-/provider",req,res);
    });

    app.get('/providers', function(req,res){
        insertQueue("GET-/providers",req,res)
    });

...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Unloadingplans", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    // api ---------------------------------------------------------------------
    app.get('/unloadingplan/:unloadingplan_id', function (req, res) {
        insertQueue("GET-/unloadingplan",req,res);
    });

    app.post('/unloadingplan', function (req, res) {
        insertQueue("POST-/unloadingplan",req,res);
    });

    app.put('/unloadingplan/:unloadingplan_id', function (req, res){
        insertQueue("PUT-/unloadingplan",req,res);
    });


    app.delete('/unloadingplan/:unloadingplan_id', function (req, res) {
        insertQueue("DELETE-/unloadingplan",req,res);
    });

    app.get('/unloadingplans', function(req,res){
        insertQueue("GET-/unloadingplans",req,res)
    });

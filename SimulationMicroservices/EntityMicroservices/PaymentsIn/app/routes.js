...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "PaymentsIn", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    // api ---------------------------------------------------------------------
    app.get('/paymentin/:paymentsin_id', function (req, res) {
        insertQueue("GET-/paymentsin",req,res);
    });

    app.post('/paymentin', function (req, res) {
        insertQueue("POST-/paymentsin",req,res);
    });

    app.put('/paymentin/:paymentsin_id', function (req, res){
        insertQueue("PUT-/paymentsin",req,res);
    });


    app.delete('/paymentin/:paymentsin_id', function (req, res) {
        insertQueue("DELETE-/paymentsin",req,res);
    });

    app.get('/paymentsin', function(req,res){
        insertQueue("GET-/paymentsins",req,res)
    });

...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Trips", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/trip/:trip_id', function (req, res) {
        insertQueue("GET-/trip",req,res);
    });

    app.post('/trip', function (req, res) {
        insertQueue("POST-/trip",req,res);
    });

    app.put('/trip/:trip_id', function (req, res){
        insertQueue("PUT-/trip",req,res);
    });

...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Operators", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/operator/:operator_id', function (req, res) {
        insertQueue("GET-/operator",req,res);
    });

    app.post('/operator', function (req, res) {
        insertQueue("POST-/operator",req,res);
    });

    app.put('/operator/:operator_id', function (req, res){
        insertQueue("PUT-/operator",req,res);
    });


    app.delete('/operator/:operator_id', function (req, res) {
        insertQueue("DELETE-/operator",req,res);
    });

    app.get('/operators', function(req,res){
        insertQueue("GET-/operators",req,res)
    });

...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Products", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/product/:product_id', function (req, res) {
        insertQueue("GET-/product",req,res);
    });

    app.post('/product', function (req, res) {
        insertQueue("POST-/product",req,res);
    });

    app.put('/product/:product_id', function (req, res){
        insertQueue("PUT-/product",req,res);
    });


    app.delete('/product/:product_id', function (req, res) {
        insertQueue("DELETE-/product",req,res);
    });

    app.get('/products', function(req,res){
        insertQueue("GET-/products",req,res)
    });

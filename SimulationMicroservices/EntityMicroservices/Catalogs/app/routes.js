...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Catalogs", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/catalog/:catalog_id', function (req, res) {
        insertQueue("GET-/catalog",req,res);
    });

    app.post('/catalog', function (req, res) {
        insertQueue("POST-/catalog",req,res);
    });

    app.put('/catalog/:catalog_id', function (req, res){
        insertQueue("PUT-/catalog",req,res);
    });


    app.delete('/catalog/:catalog_id', function (req, res) {
        insertQueue("DELETE-/catalog",req,res);
    });

    app.get('/catalogs', function(req,res){
        insertQueue("GET-/catalogs",req,res)
    });

...
function messageHandler(msg){
    if(msg.type == "start"){
        ...
        .send({ "simID": request.req.body.simID, "opID": request.req.body.opID, "step" : request.req.body.step+1, 
            "timing" : msElapsed ,"serviceName": "Warehouses", "apiName": request.type,"date":datestring,
...
module.exports = function (app) {
    ...
    app.get('/warehouse/:warehouse_id', function (req, res) {
        insertQueue("GET-/warehouse",req,res);
    });

    app.post('/warehouse', function (req, res) {
        insertQueue("POST-/warehouse",req,res);
    });

    app.put('/warehouse/:warehouse_id', function (req, res){
        insertQueue("PUT-/warehouse",req,res);
    });


    app.delete('/warehouse/:warehouse_id', function (req, res) {
        insertQueue("DELETE-/warehouse",req,res);
    });

    app.get('/warehouses', function(req,res){
        insertQueue("GET-/warehouses",req,res)
    });
